...
## scale_time - is TRUE if the time-varying covariate should be scaled, FALSE otherwise
...
gbtm_monotone <- function(data, n_gps, x, poly_degs = rep(3, n_gps), n_starts = 5, monotone = TRUE, scale_time = TRUE, covariates = NULL, plot_it = TRUE, conf_level = NULL, boot = 'off', sims = 'off', response = 'gaussian'){
  ...
  if(boot == 'off'){                                                   ## if we are not in a bootstrapped run (see conf_boot)
    ...
    if(is.vector(x)){                                                  ## if time-varying covariate is the same for all individuals
      ...
      if(scale_time) x <- (x - mean(x))/sd(x)                          ## scale the time varying covariate for computation
      X_des <- matrix(t(matrix(x, nrow = TT, ncol = p+1))^(0:p),       ## create design matrix X_des
                      nrow = TT*n,
                      ncol = p+1,
                      byrow = TRUE)
      ...
      x_unique <- sort(unique(X_des_unscl))
      if(scale_time){
        x_tmp <- (x_unique - mean(x_unique))/sd(x_unique)              ## scale the unique times
      } else {
        x_tmp <- x
      }
      X_des <- vector(length = length(X_des_unscl))                    ## set up scaled design matrix
      for(i in 1:length(x))
        X_des[X_des_unscl == x_unique[i]] <- x_tmp[i]                  ## set each element to appropriate scaled time
